...
- `--browser-thinking-time <light|standard|extended|heavy|pro>`: set the ChatGPT thinking-time intensity. `pro` selects and verifies the unified picker's top Pro effort and requires GPT-5.6 Sol with model strategy `select`. You can also set a default in `~/.oracle/config.json` via `browser.thinkingTime`.
