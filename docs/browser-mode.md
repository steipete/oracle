...
- `--project-sources <add|delete|replace|sync>`: manage a ChatGPT project's persistent Sources tab directly. Use `--file` for uploads, `--source` for delete targets, and point `--chatgpt-url` at the target project URL. Uploads are batched in groups of 10 files because ChatGPT caps each Sources upload at 10 files.
