...
2. **Automation stack** – code lives under `src/browser/`:
   ...
   - When `--file` inputs would push the pasted composer content over ~60k characters, we switch to uploading attachments and wait for ChatGPT to re-enable the send button before submitting the combined system+user prompt. Multiple text files become one ordered, source-labelled text bundle automatically; one text file keeps its original attachment name.
...
- `--browser-bundle-files`: explicitly bundle all resolved attachments into a single temp file before uploading (only used when uploads are enabled/selected). Without this flag, multiple uploaded text files are still combined automatically as one ordered, source-labelled text corpus; raw files remain direct uploads unless the attachment limit requires a ZIP.
...
- **Attachment lifecycle** – in `auto` mode we prefer inlining files into the composer (fewer moving parts). When uploads are selected, one text file keeps its original attachment name and multiple text files become one ordered bundle with source labels and line numbers. Raw files remain byte-preserving direct uploads unless explicit bundling or the attachment limit requires a ZIP. The automation waits for uploads to finish (send button enabled, upload chips visible) before submitting. When inline paste is rejected by ChatGPT (too large), Oracle retries automatically with the same upload policy.
