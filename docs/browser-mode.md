...
- `--browser-max-concurrent-tabs`: soft limit for simultaneous ChatGPT tabs sharing one manual-login profile (default `3`). Set `ORACLE_BROWSER_MAX_CONCURRENT_TABS` for a per-host default; explicit CLI/config values win. Additional runs wait in FIFO order and log `[browser] Waiting for ChatGPT browser slot...`.
- `--browser-queue-timeout`: independent queue wait budget (default `20m`). Set `0` to wait indefinitely without consuming the active browser/input budgets. `ORACLE_BROWSER_QUEUE_TIMEOUT` and `browser.queueTimeoutMs` configure the same limit.
...
When Codex, Claude Code, or another Oracle caller share the same manual-login profile, each browser run acquires a tab slot before opening a ChatGPT tab. The default allows three simultaneous ChatGPT tabs; additional callers wait in FIFO order. Operators who need strict serialization for long Pro/Thinking runs can set the limit to `1`.

Use `--browser-max-concurrent-tabs <n>`, `browser.maxConcurrentTabs`, or `ORACLE_BROWSER_MAX_CONCURRENT_TABS` to tune the soft limit. Precedence is explicit CLI/config value, then environment, then the default of `3`; invalid or non-positive environment values fall back instead of disabling the cap. Waiters are persisted in FIFO order. Queue time is separate from active browser time and defaults to `20m`; use `--browser-queue-timeout`, `browser.queueTimeoutMs`, or `ORACLE_BROWSER_QUEUE_TIMEOUT` to change it, with `0` opting into an unlimited wait. Higher tab limits are an explicit reliability tradeoff: too many concurrent ChatGPT tabs can make the UI unstable or trigger account-side throttling. Oracle also serializes manual-login Chrome startup for the shared profile, then reuses the first reachable DevTools session instead of racing multiple Chrome launches against the same `user-data-dir`. The short profile lock still serializes the send/upload moment so separate agents do not type into the same composer.
