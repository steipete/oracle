...
- `--browser-scheduled-task`: create a recurring task through ChatGPT's Scheduled page. Oracle targets `/scheduled`, skips the model picker, keeps the source conversation, and verifies the handoff to the task conversation. The prompt must contain the title, schedule, and monitoring instructions. This mode cannot be combined with Deep Research, follow-ups, or browser archiving.
- `--browser-pin-conversation`: pin the resulting ChatGPT conversation after a successful run. Oracle fails closed unless the sidebar changes to the verified `Unpin` state.
