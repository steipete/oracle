...
2. **Automation stack** – code lives under `src/browser/`:
   ...
   - Immediately probes `/backend-api/me` in the ChatGPT tab to verify the session is authenticated; because this legacy cookie-only probe can return 401/403 for signed-in remote Chrome sessions, Oracle accepts those responses only when the page also shows a visible composer plus profile or chat-history authentication signals. Auth pages, visible login controls, composer-only shells, and pages without profile/history signals still fail with login guidance.
