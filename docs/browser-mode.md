...
- GPT-5.5 Pro Extended is verified from ChatGPT's "Intelligence" picker (the checked `Pro Extended` radio), and a run **fails closed** if that effort can't be confirmed, rather than silently submitting at a weaker effort. Set `ORACLE_BROWSER_PRO_EFFORT_RELAXED=1` (or `true`/`yes`/`on`) to opt out: an unconfirmed Pro Extended effort then downgrades to a warning and the run continues at ChatGPT's current effort (the Pro model already defaults to Pro Extended). On any thinking-time detection failure Oracle writes a model-picker diagnostic to the session log so UI drift can be diagnosed without re-running with `--verbose`.
