...
- `--copy-profile <dir>`: copy a signed-in Chrome user-data directory (e.g. `"$HOME/Library/Application Support/Google/Chrome"`) to a throwaway profile and run against it, reusing your live ChatGPT session with no manual sign-in. The copy is launched with the real Keychain (not mocked) so its encrypted cookies decrypt, and is always deleted afterward (including on interrupt). Not compatible with `--browser-keep-browser` (the copy is a throwaway), and fails fast if the required `Local State` cannot be copied. macOS/Linux; requires `rsync`.
