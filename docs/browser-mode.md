...
- `oracle import-chatgpt-url <url> --slug "<words>"`: register a manually started ChatGPT conversation as a completed browser session so later `--followup <slug>` runs can continue it through the same fail-closed resume path.
