...
- Each submitted Pro turn stores its own dispatch timestamp, response elapsed time, estimated input tokens, uploaded bytes, normalized prompt digest, and exact committed user-turn index. Elapsed time is durable evidence, not a default answer-rejection threshold: a fast response remains valid when the receipt is complete. Reattach verifies the stored prompt digest at the stored turn index before accepting an answer, so a follow-up cannot borrow the initial turn's workload or recover from a neighboring conversation turn. Marker-free sessions created by older versions remain recoverable; once any new-format marker is present, every dispatch, timing, workload, commit, digest, and turn-index field is required and incomplete evidence fails closed.
