...
- `--browser-max-concurrent-tabs`: soft limit for simultaneous ChatGPT tabs sharing one manual-login profile (default `1`). Set `ORACLE_BROWSER_MAX_CONCURRENT_TABS` for a per-host default; explicit CLI/config values win. Additional runs wait in FIFO order and log `[browser] Waiting for ChatGPT browser slot...`.
- `--browser-queue-timeout`: independent queue wait budget. The default `0` waits indefinitely, so time spent behind a long Pro run does not consume the active browser/input budgets. Set `ORACLE_BROWSER_QUEUE_TIMEOUT` or `browser.queueTimeoutMs` to bound queueing explicitly.
...
When Codex, Claude Code, or another Oracle caller share the same manual-login profile, each browser run acquires a tab slot before opening a ChatGPT tab. The default serializes runs so the active response and artifacts finish before the next caller takes over the shared Chrome profile. This is the reliable path for long Pro/Thinking runs; users who have validated parallel browser behavior for their account can opt into a higher limit.

Use `--browser-max-concurrent-tabs <n>`, `browser.maxConcurrentTabs`, or `ORACLE_BROWSER_MAX_CONCURRENT_TABS` to tune the soft limit. Precedence is explicit CLI/config value, then environment, then the default of `1`; invalid or non-positive environment values fall back instead of disabling the cap. Waiters are persisted in FIFO order. Queue time is separate from active browser time; use `--browser-queue-timeout`, `browser.queueTimeoutMs`, or `ORACLE_BROWSER_QUEUE_TIMEOUT` only when a bounded queue wait is desired. Higher tab limits are an explicit reliability tradeoff: too many concurrent ChatGPT tabs can make the UI unstable or trigger account-side throttling. Oracle also serializes manual-login Chrome startup for the shared profile, then reuses the first reachable DevTools session instead of racing multiple Chrome launches against the same `user-data-dir`. The short profile lock still serializes the send/upload moment so separate agents do not type into the same composer.
