...
- Cookie controls:
  ...
  - **Automation/agent use:** on macOS, reading a live Chrome profile's cookies requires a one-time interactive "Always Allow" approval in Keychain Access; this only works from a real human-attended terminal session. When Oracle detects no interactive session (no TTY on stdin/stdout, or `CI`/`ORACLE_NONINTERACTIVE=1` set), it fails fast with a `--browser-manual-login` recommendation instead of hanging on a Keychain prompt nobody can answer. Use `--browser-manual-login` (a persistent, Keychain-free automation profile) for agent/automation-invoked runs; reserve cookie-copy mode for interactive human CLI use. Set `ORACLE_ASSUME_INTERACTIVE=1` only if you know a human is present to approve the Keychain prompt despite the environment looking non-interactive.
