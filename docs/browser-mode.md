...
- `--browser-thinking-time <light|standard|extended|heavy|pro>`: set the ChatGPT thinking-time intensity (Thinking/Pro models only). `pro` selects and verifies the unified picker's top Pro effort. You can also set a default in `~/.oracle/config.json` via `browser.thinkingTime`.
