...
- `--browser-thinking-time <light|standard|extended|heavy|pro>`: set the ChatGPT thinking-time intensity. `pro` is an explicit CLI-only selection for GPT-5.6 Sol with model strategy `select`; the other levels can be defaults in `~/.oracle/config.json` via `browser.thinkingTime`.
