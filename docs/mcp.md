...
- Browser file uploads: one text/source file stays native, while multiple text/source files default to one byte-preserving ZIP plus a short extraction instruction. Native images and documents remain separate when possible. Set `browserBundleFiles:true` to force one all-file ZIP under `auto`/`zip`, or `browserBundleFormat:"text"` to retain the legacy flattened text bundle.
