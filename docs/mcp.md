...
- Dry runs: set `dryRun: true` to preview the resolved request without creating a session or touching the browser. Browser dry-run responses may include `recommendedConversationMode?: "one-shot"|"multi-turn"|"deep-research"|"project"`; this is advisory only and never changes routing, creates follow-ups, or mutates ChatGPT Projects.
...
- Project guidance: if a ChatGPT Project URL is configured, dry-run may recommend `project` mode. Project Sources are persistent until the user removes them, and Developer Mode / Memory Off is recommended for agent-originated browser consults so prompts do not become ChatGPT memory.
