    ...
    try {
      ...
      await active.release({
        onRelease: async ({ isLastLease }) => {
          const registry = JSON.parse(
            await readFile(path.join(dir, "oracle-tab-leases.json"), "utf8"),
          ) as { version: number; leases: unknown[]; waiters: Array<{ sessionId?: string }> };
          releaseSnapshot({ isLastLease, registry });
        },
      });
      expect(releaseSnapshot).toHaveBeenCalledWith({
        ...
        registry: expect.objectContaining({
          version: 2,
          leases: [],
          waiters: [expect.objectContaining({ sessionId: "queued-session" })],
        }),
      });
