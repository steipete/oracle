...
function buildTabInspectionExpression(): string {
  ...
  return `(() => {
      ...
      const searchUnitRole = (turn) => {
        const key = turn.getAttribute('data-chatgpt-search-unit-key') || '';
        return key.split(':').at(-1) || '';
      };
      const assistantTurns = turns.filter((turn) => {
        const role = normalize(turn.getAttribute('data-message-author-role') || turn.getAttribute('data-turn') || searchUnitRole(turn)).toLowerCase();
        if (role === 'assistant') return true;
        return Boolean(turn.querySelector(ASSISTANT_ROLE_SELECTOR));
      });
      const fallbackUserTurns = Array.from(
        document.querySelectorAll('[data-message-author-role="user"], [data-turn="user"], [data-chatgpt-search-unit-key$=":user"]'),
      );
      ...
      const lastUserMessage = lastUserTurn?.matches?.('[data-message-author-role="user"], [data-turn="user"], [data-chatgpt-search-unit-key$=":user"]')
        ? lastUserTurn : lastUserTurn?.querySelector?.('[data-message-author-role="user"], [data-turn="user"], [data-chatgpt-search-unit-key$=":user"]');
      ...
      return {
        ...
        lastUserMessageId:
          lastUserMessage?.getAttribute?.('data-message-id') ||
          lastUserMessage?.getAttribute?.('data-chatgpt-search-message-ids'),
