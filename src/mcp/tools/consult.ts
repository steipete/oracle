...
const consultInputShape = {
  ...
  browserThinkingEffort: z
    .enum(['standard', 'extended'])
    .optional()
    .describe('Browser-only: set ChatGPT thinking effort (Standard/Extended) when supported by the chosen model.'),
  ...
  browserKeepTabs: z
    .boolean()
    .optional()
    .describe('Browser-only: keep the ChatGPT tab after completion (default is to auto-close).'),
  search: z
    .boolean()
    .optional()
    ...
    async (input: unknown) => {
      ...
      if (resolvedEngine === 'browser') {
        ...
        browserConfig = {
          ...
          keepTabs: browserKeepTabs ?? false,
          manualLogin,
          manualLoginProfileDir: manualLogin ? envProfileDir : null,
          thinkingEffort: browserThinkingEffort ?? null,
