...
function renderHelpFooter(program: Command, colors: HelpColors): string {
  const tips = [
    ...
    `${colors.bullet("•")} Oracle is one-shot by default. API runs can chain follow-ups via ${colors.accent("--followup <sessionId|responseId>")} ; browser runs can continue a stored ChatGPT/GPT browser session via ${colors.accent("--followup <sessionId>")}.`,
