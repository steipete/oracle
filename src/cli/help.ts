...
function renderHelpBanner(version: string, colors: HelpColors): string {
  const subtitle = 'Prompt + files required — GPT-5.4 Pro/GPT-5.2 for tough questions with code/file context.';
...
function renderHelpFooter(program: Command, colors: HelpColors): string {
  ...
  const examples = [
    ...
    formatExample(
      `${program.name()} --prompt "Cross-check the data layer assumptions" --models gpt-5.4-pro,gemini-3-pro --file "src/**/*.ts"`,
