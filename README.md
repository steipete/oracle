...
- Dry-run guidance: browser dry-runs recommend a conversation mode (`one-shot`, `multi-turn`, `deep-research`, or `project`) so agents can explain whether a temporary consult or persistent ChatGPT Project context fits the request. The recommendation is advisory only; Oracle does not switch projects, create follow-ups, or upload Project Sources unless the caller explicitly asks for those actions.
- AGENTS.md/CLAUDE.md:
  ```
