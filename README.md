...
- ChatGPT Project Sources: browser mode can now manage a project's persistent Sources tab directly via `--project-sources add|delete|replace|sync` plus `--file` / `--source`, and MCP exposes the same flow via the `project_sources` tool.
...
| `--project-sources <add\|delete\|replace\|sync>`                | Manage a ChatGPT project's persistent Sources tab directly in browser mode; pair with `--file` for uploads and `--source` for delete targets.                                                                                                                                                                  |
| `--source <names...>`                                           | Source names to delete/replace when using `--project-sources`.                                                                                                                                                                                                                                                   |
